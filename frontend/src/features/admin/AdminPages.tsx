...
function getAdminSectionContent(section: AdminSection, currentUser: CurrentUser) {
  const scope = (currentUser.user_type.name === "super_admin" || currentUser.user_type.name === "root") ? "全部实验室" : describeScope(currentUser);
...
function LaboratoriesSection({ currentUser }: { currentUser: CurrentUser }) {
  ...
  const isOwner = userTypeName === "super_admin" || userTypeName === "root";
...
function UsersSection({ currentUser }: { currentUser: CurrentUser }) {
  ...
  function canEditUser(_user: AdminUser) {
    if (isOwner) {
      return true;
    }
    return false;
...
function getRoleOptions(currentUserType: string) {
  if (currentUserType === "super_admin" || currentUserType === "root") {
    return ["super_admin", "lab_admin", "user", "guest"];
  }
  if (currentUserType === "lab_admin") {
...
function requiresLaboratory(roleName: string) {
  return ["lab_admin", "user"].includes(roleName);
}
