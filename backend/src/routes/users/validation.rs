...
use crate::authentication::{Actor, LAB_ADMIN, ROOT, SUPER_ADMIN, USER, requires_laboratory, user_type_exists};
...
pub(super) fn resolve_target_laboratory(
    ...
    if matches!(target_user_type, LAB_ADMIN | USER) {
        // lab_admin and user require a laboratory
        if let Some(lab) = requested_laboratory_id {
            return Ok(Some(lab));
        }
        // If no lab specified, use actor's lab (for scoped admins)
        return Ok(actor.laboratory_id);
    }
    // super_admin and guest do not require a laboratory
    Ok(requested_laboratory_id)
}

pub(super) fn ensure_can_view_user(actor: &Actor, target: &UserRow) -> Result<(), ApiError> {
    if actor.is_admin() || actor.user_id == target.user_id {
        return Ok(());
    }
...
pub(super) fn normalize_user_type(user_type: &str) -> Result<String, ApiError> {
    ...
    if matches!(user_type, ROOT | SUPER_ADMIN | LAB_ADMIN | USER | "guest") {
