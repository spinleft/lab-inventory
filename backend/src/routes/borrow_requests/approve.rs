use super::helpers::{
    ...
    record_inventory_transaction, update_inventory_after_approval,
};
...
#[tracing::instrument(name = "Approve a borrow request", skip(request, pool, payload), fields(user_id=%user_id, borrow_request_id=%borrow_request_id))]
pub async fn approve_borrow_request(
    ...
    match try_processing(pool.get_ref(), &idempotency_key, *user_id).await? {
        ...
        NextAction::StartProcessing(mut transaction) => {
            ...
            if borrow_request.requested_quantity > available_quantity {
                return Err(ApiError::BadRequest(
            ...
            update_inventory_after_approval(
                &mut transaction,
                &item,
