...
#[tracing::instrument(name = "Allocate an inventory item", skip(request, pool, payload), fields(user_id=%user_id, inventory_item_id=%inventory_item_id))]
pub async fn allocate_inventory_item(
    request: HttpRequest,
    user_id: UserId,
    ...
    if existing.tracking_mode == "serialized" && payload.quantity != 1.0 {
        return Err(ApiError::BadRequest(
            "serialized inventory allocation quantity must be 1".into(),
        ));
    }
    let new_allocated = existing.quantity_allocated + payload.quantity;
    if new_allocated > existing.quantity_on_hand {
        return Err(ApiError::BadRequest(
            "allocated quantity cannot exceed quantity_on_hand".into(),
        ));
    }
