...
#[tracing::instrument(name = "Stocktake an inventory item", skip(request, pool, payload), fields(user_id=%user_id, inventory_item_id=%inventory_item_id))]
pub async fn stocktake_inventory_item(
    request: HttpRequest,
    user_id: UserId,
    ...
    if existing.tracking_mode == "serialized" && payload.quantity_on_hand != 1.0 {
    ...
    if payload.quantity_on_hand < existing.quantity_allocated {
        return Err(ApiError::BadRequest(
            "quantity_on_hand cannot be lower than quantity_allocated".into(),
        ));
    }
    let quantity_delta = payload.quantity_on_hand - existing.quantity_on_hand;
        ...
        NextAction::StartProcessing(mut transaction) => {
            ...
            record_inventory_transaction(
                ...
                InventoryTransactionData {
                    ...
                    details: json!({ "quantity_on_hand": item.quantity_on_hand }),
                },
            )
            .await?;
            record_audit(
                ...
                json!({ "quantity_on_hand": item.quantity_on_hand }),
            )
            .await?;
